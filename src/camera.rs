...
pub struct CameraBuilder {
    /// The ratio of width over height of the image.
    pub aspect_ratio: Option<f64>,
    /// The image width in pixels.
    pub image_width: Option<u32>,
    /// Count of random samples per pixel
    pub samples_per_pixel: Option<u32>,
    /// The limit to hit if the number of ray bounces exceeds this amount.
    pub max_depth: Option<u32>,
    /// Angle from origin (camera location) to viewport top and bottom (usually 90 deg).
    pub vertical_fov: Option<f64>,
    /// Where the camera is positioned (usually at the origin).
    pub look_from: Option<DVec3>,
    /// The point the camera is looking toward.
    pub look_at: Option<DVec3>,
    /// The roll around the look_from/look_at axis (which way is camera "up").
    pub v_up: Option<DVec3>,
    /// Defocus angle is 0 when all rays originate from camera origin.
    /// To avoid varying the blur based on projection distance,
    /// this value is the angle of the cone whose apex is at the viewport
    /// center and whose base is at the camera center. (the base at camera center
    /// being like the lens).
    /// The larger this value, the greater the defocus blur.
    pub defocus_angle: Option<f64>,
    /// The distance from the camera to the plane where objects are
    /// perfectly in focus.
...
impl CameraBuilder {
    ...
    pub fn build(self) -> Camera {
        ...
        let vertical_fov = self.vertical_fov.unwrap_or(20.);
        let look_from = self.look_from.unwrap_or(DVec3::new(0., 0., -1.));
        let look_at = self.look_at.unwrap_or(DVec3::ZERO);
        let v_up = self.v_up.unwrap_or(DVec3::Y);
        let defocus_angle = self.defocus_angle.unwrap_or(0.);
        let focus_dist = self.focus_dist.unwrap_or((look_from - look_at).length());
...
pub struct Camera {
    ...
    // The center of the top right-most pixel on the pixel plane
    pixel_00_loc: DVec3,
    defocus_angle: f64,
...
impl Camera {
    ...
    pub fn render(
        ...
        let pixels: Vec<DVec3> = (0..(self.image_height as u32))
            ...
            .enumerate()
            .map(|(i, y)| {
                let row: Vec<DVec3> = (0..(self.image_width as u32))
                    .map(|x| {
    ...
    /// [-.5,-.5]-[+.5,+.5] unit square.
    /// This provides anti-aliasing to the image
